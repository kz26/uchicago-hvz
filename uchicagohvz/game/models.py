...
class Squad(models.Model):
	...
	def human_points(self):
		if self.get_active_players().count() > 0:
			return 10.0 * sum([p.human_points for p in self.players.filter(active=True, human=True)]) / self.get_active_players().count()
		return 0

	@property
	def zombie_points(self):
		if self.get_active_players().count() > 0:
			return 10.0 * sum([p.zombie_points for p in self.players.filter(active=True, human=False)]) / self.get_active_players().count()
		return 0
