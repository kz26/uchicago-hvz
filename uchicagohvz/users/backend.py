from django.contrib.auth import get_user_model, authenticate as auth
...
class RhodesUniLDAPBackend(object):
	"""
	Authenticates against settings.AUTH_USER_MODEL.
	"""
	LDAP_SERVER = "ldap://ldap.ru.ac.za/"
	...
	def bind(self, cnetid, password):
		cnetid = ldap.filter.escape_filter_chars(cnetid)
		try:
			self.conn.simple_bind_s("uid=%s,ou=STUDENT,o=RU" % cnetid, password)
		except ldap.LDAPError:
			return False
		return True
	...
	def get_user_data(self, cnetid): # look up a user by CNetID
		cnetid = ldap.filter.escape_filter_chars(cnetid)
		query = "(&(uid=%s)(objectclass=person))" % (cnetid)
		results = self.conn.search_ext_s("o=RU", ldap.SCOPE_SUBTREE, query)
		if results:
			user_data = results[0][1]
			if not user_data.get('uid'):
				return None
			...
			if not user_data.get('sn'):
				user_data['sn'] = ['Unknown']
			if not user_data.get('mail'):
				user_data['mail'] = ["%s@campus.ru.ac.za" % (cnetid)]
			return user_data
		else:
			return None
